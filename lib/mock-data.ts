...
/** "My Tickets" membership in mock data: does `login` hold one of the chosen
 *  roles on this ticket? Mock summaries carry assignee + reporter (no cc), so
 *  cc is a no-match here — fine for the offline fallback. */
function matchesInvolves(t: TicketSummary, login: string, roles?: string[]): boolean {
  const r = roles && roles.length ? roles : ["assignee"];
  return (r.includes("assignee") && t.assignee === login) ||
         (r.includes("reporter") && t.reporter === login);
}

export function mockSearch(opts: {
  product?: string; component?: string;
  status?: string | string[]; severity?: string | string[];
  assignee?: string; involves?: string; involveRoles?: string[]; q?: string; limit?: number;
  ...
  const filtered = MOCK_SUMMARIES.filter(t =>
    ...
    (!opts.involves || matchesInvolves(t, opts.involves, opts.involveRoles)) &&
...
export function buildMockStats(opts: {
  product?: string; component?: string; assignee?: string; involves?: string; involveRoles?: string[];
}): DashboardStats {
